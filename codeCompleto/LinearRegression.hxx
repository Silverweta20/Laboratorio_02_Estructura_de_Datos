...
void LinearRegression<T>::calculateMAE(const DataSet<T> &dataSet)
{
    ...
    int n=dataSet.dataPoints.size();

    typename std::deque<DataPoint<T>>::const_iterator it=dataSet.dataPoints.begin();


    double acumulador=0.0;
    double resta;

    for(;it!=dataSet.dataPoints.end();it++){

        DataPoint<T> predicted = this->predict(*it);

        resta=it->y - predicted.y;
    ...
    this->MAE = acumulador / n;
    this->MAECalculated = true;
...
void LinearRegression<T>::calculateMSE(const DataSet<T> &dataSet)
{
    ...
    for (; it != dataSet.dataPoints.end(); ++it) {
        ...
        error = it->y - predicted.y;

        acumulador = acumulador + (error * error);
    }

    this->MSE = acumulador / n;
    this->MSECalculated = true;
...
void LinearRegression<T>::calculateRMSE(const DataSet<T> &dataSet)
{
    ...
    if (!this->MSECalculated){
        this->calculateMSE(dataSet);
    }
    
    this->RMSE = std::sqrt(this->MSE);
    this->RMSECalculated = true;
...
DataPoint<T> LinearRegression<T>::predict(const DataPoint<T> &inputPoint) const
{
    ...
    return DataPoint<T>(inputPoint.x, inputPoint.x * this->slope + this->intercept);
